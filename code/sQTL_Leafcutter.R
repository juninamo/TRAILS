...
cov = as.data.frame(fread(paste("/path/to/DICE/Imputed/QC_P3_R20.7_MAF0.05_HWE1E06_nodup.hg38_PC10.txt",sep=""),header = TRUE)) %>%
  ...
  .[,c("id",read.table("/path/to/DICE/kallisto/sample_list.txt", header = TRUE)[,1])]
...
IID_tr_g <- 
  ...
  .[read.table("/path/to/GEUV/kallisto/sample_list.txt", header = TRUE)[,1],] %>%
...
isopos_c = fread("/path/to/leafcutter/isoform_position.txt") %>% 
  as.data.frame()
psi_c <- fread(paste0("/path/to/sQTL/leafcutter/quantile-rank-normalized_psi.txt")) %>%
  dplyr::rename(gid = V1) %>%
  as.data.frame()
peer_factors <- fread(paste0("/path/to/sQTL/leafcutter/PEERfactors.txt")) %>%
  as.data.frame()


chr = commandArgs(trailingOnly=TRUE)[1] %>% as.integer() #1番目の引数を取得
# chr=1
...
for (stim in c("TFH","TH1","TH17","TH2","TH1-17","TREG_MEMORY","TREG_NAIVE","B_NAIVE","CD4_NAIVE","CD4_N_STIM","CD8_NAIVE","CD8_N_STIM","NONCLASSICAL_MONOCYTES","CLASSICAL_MONOCYTES","NK_CD16POS")){
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      write.table(snps, paste0("/path/to/snps_chr",chr,"_leafcuttter.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(expr2, paste0("/path/to/exp_chr",chr,"_leafcuttter.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(cov, paste0("/path/to/cov_chr",chr,"_leafcuttter.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      
      # Linear model to use, modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      useModel = modelLINEAR; # modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      
      # Genotype file name
      SNP_file_name = paste("/path/to/snps_chr",chr,"_leafcuttter.txt", sep="");
      
      # Gene expression file name
      expression_file_name = paste("/path/to/exp_chr",chr,"_leafcuttter.txt", sep="");
      
      # Covariates file name
      # Set to character() for no covariates
      covariates_file_name = paste("/path/to/cov_chr",chr,"_leafcuttter.txt", sep="");
      ...
      me$cis$eqtls %>%
        dplyr::inner_join(.,
                          ...
                          by="snps") %>%
        dplyr::mutate(chrom = paste0("chr",str_split(snps, pattern = "_", simplify = TRUE) %>% .[,1]),
                      start = str_split(gene, pattern = ":", simplify = TRUE) %>% .[,2] %>% as.integer(),
                      end = str_split(gene, pattern = ":", simplify = TRUE) %>% .[,3] %>% as.integer(),
                      width = abs(end-start),
                      strand = str_split(gene, pattern = "_", simplify = TRUE) %>% .[,3]
        ) %>%
        valr::bed_closest(., ref_df, suffix = c("_sQTL", "_ref")) %>%
        as.data.frame() %>%
        .[,c("chrom","start_sQTL","end_sQTL","strand_sQTL","snps_sQTL","gene_sQTL","statistic_sQTL", "pvalue_sQTL","FDR_sQTL" ,"beta_sQTL","start_ref","end_ref", "class_ref", "gene_ref", ".overlap", ".dist", "A1_sQTL", "A0_sQTL")] %>%
        magrittr::set_colnames(c("chrom","start_clu","end_clu","strand_clu","snp","clu","statistic", "pval","FDR" ,"beta","start_gene","end_gene", "class", "gene", "overlap", "dist", "A1", "A0")) %>%
        dplyr::arrange(pval) %>%
        write.table(., 
                    paste0("/path/to/DICE/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_leafcuttter.txt"),
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pval) %>%
        write.table(., 
                    paste0("/path/to/EGA/QTL/result/splicing_peer/chr",chr,"_in_",stimulus,"_leafcuttter.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pval) %>%
        write.table(., 
                    paste0("/path/to/ImmVar/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_leafcuttter.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pval) %>%
        write.table(., 
                    paste0("/path/to/GEUV/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_leafcuttter.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
