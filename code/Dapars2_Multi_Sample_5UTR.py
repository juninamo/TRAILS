...
def Convert_wig_into_bp_coverage(extracted_coverage,extracted_5UTR_region,strand_info):
    bp_coverage = np.zeros(extracted_5UTR_region[-1] - extracted_5UTR_region[0])
    relative_start = extracted_5UTR_region[0]
    for i in range(len(extracted_coverage)):
        curr_region_start = extracted_5UTR_region[i] - relative_start
        curr_region_end = extracted_5UTR_region[i+1] - relative_start
...
def parse_cfgfile(cfg_file):
        ...
        if line[0] == '\n' or line[0] == '#':
            ...
            if command[0] == 'Output_directory':
                ...
                if output_directory[-1] != '/':
                    output_directory += '/'
            if command[0] == 'Annotated_5UTR':
                Annotated_5UTR_file = command[1]
    ...
    if output_directory=='':
        ...
        exit(1)
    if Annotated_5UTR_file=='':
        print >> sys.stderr, "No annotated 5' UTR file!"
        exit(1)
    ...
    return Aligned_Wig_files, output_directory, Annotated_5UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold
...
def De_Novo_5UTR_Identification_Loading_Target_Wig_for_TCGA_Multiple_Samples_Multiple_threads_Main3_shared_list(argv=None):
    ...
    Group1_Tophat_aligned_file, output_directory, Annotated_5UTR_file, Output_result_file, sequencing_depth_file, Num_threads, Coverage_threshold = parse_cfgfile(cfg_file)
    ...
    All_samples_Target_5UTR_coverages, UTR_events_dict = Load_Target_Wig_files_Multiple_threads_shared_dict_sampleid_key(All_Sample_files, Annotated_5UTR_file, Num_threads,curr_processing_chr)
    ...
    for i in range(num_real_threads):
        curr_temp_output = temp_dir + 'Each_processor_5UTR_Result_%s.txt' % (str(i+1))
        Output_each_processor_all.append(curr_temp_output)

    processes = []
    for i in range(num_real_threads):
        process = multiprocessing.Process(target=Each_Thread_5UTR_estimation_list_version_sample_ids, args=(Assigned_events_ids_all_threads[i], UTR_events_dict, All_sample_coverage_weights, num_samples, Output_each_processor_all[i], All_samples_Target_5UTR_coverages, Coverage_threshold))
        process.start()
...
def Each_Thread_5UTR_estimation_list_version_sample_ids(curr_thread_UTR_events_ids, UTR_events_dict, All_sample_coverage_weights, num_samples, Output_result_file, All_samples_coverage_shared_dict, Coverage_threshold):
    Output_result = open(Output_result_file,'w')

    for curr_5UTR_id in curr_thread_UTR_events_ids:
        curr_5UTR_structure = UTR_events_dict[curr_5UTR_id]
        region_start = curr_5UTR_structure[1]
        region_end = curr_5UTR_structure[2]
        curr_strand = curr_5UTR_structure[-2]
        UTR_pos = curr_5UTR_structure[-1]
        curr_5UTR_all_samples_bp_coverage = []

        for i in range(num_samples):
            curr_sample_curr_5UTR_coverage_wig = All_samples_coverage_shared_dict[curr_5UTR_id, i]
            curr_5UTR_curr_sample_bp_coverage = Convert_wig_into_bp_coverage(curr_sample_curr_5UTR_coverage_wig[0], curr_sample_curr_5UTR_coverage_wig[1], curr_strand)
            curr_5UTR_all_samples_bp_coverage.append(curr_5UTR_curr_sample_bp_coverage)

        select_mean_squared_error, selected_break_point, UTR_abundances = De_Novo_5UTR_Coverage_estimation_Genome_for_multiple_samples(curr_5UTR_all_samples_bp_coverage, region_start, region_end,curr_strand,All_sample_coverage_weights, Coverage_threshold)

        if str(select_mean_squared_error) != "Na":
            num_non_zero = 1
            if num_non_zero > 0:
                All_long_inclusion_ratios = []
                line_write = [curr_5UTR_id, "%.1f" % select_mean_squared_error, str(selected_break_point), UTR_pos]

                for i in range(num_samples):
                    if UTR_abundances[0][i] != 'NA':
                        # long 5'UTR percentage
                        ...
                        #line_write.append("%.2f" % UTR_abundances[0][i])#long 5' UTR abundance
                        #line_write.append("%.2f" % UTR_abundances[1][i])#short 5' UTR abundance
...
def De_Novo_5UTR_Coverage_estimation_Genome_for_multiple_samples(All_Samples_curr_5UTR_coverages, UTR_start, UTR_end, curr_strand, weight_for_second_coverage, Coverage_threshold):
    coverage_threshold = Coverage_threshold
    search_point_start = 150 ##200
    search_point_end = int(abs((UTR_end - UTR_start))*0.05)

    num_samples = len(All_Samples_curr_5UTR_coverages)
    #Read Coverage
    Region_Coverages = []
    Pass_threshold_index = []
    for i in range(num_samples):
        curr_Region_Coverage_raw = All_Samples_curr_5UTR_coverages[i]
    ...
    if len(Pass_threshold_index) > least_pass_coverage_num and UTR_end - UTR_start >=150:
        ...
        for curr_point in range(search_region_start, search_region_end+1):
            ...
            Estimated_5UTR_abundance_list.append([All_samples_result[1],All_samples_result[2]])
        ...
        if len(Mean_squared_error_list) > 1:
            ...
            UTR_abundances_passed = Estimated_5UTR_abundance_list[min_ele_index]
...
def Load_Target_Wig_files_Multiple_threads_shared_dict_sampleid_key(All_Wig_files,UTR_Annotation_file, num_threads,curr_processing_chr):
    ...
    All_samples_extracted_5UTR_coverage_dict = manager.dict() # create only 1 dict

    processes = []
    Final_assigned_threads_num = len(Assigned_index)
    for i in range(Final_assigned_threads_num):
        process = multiprocessing.Process(target=load_wig_funct_shared_dict_sampleid_key, args=(All_Wig_files, Assigned_index[i], UTR_events_dict,curr_processing_chr,All_samples_extracted_5UTR_coverage_dict))
        process.start()
        processes.append(process)
    ...
    return All_samples_extracted_5UTR_coverage_dict, UTR_events_dict


def load_wig_funct_shared_dict_sampleid_key(All_wig_files, assigned_indexes,UTR_events_dict, curr_processing_chr, All_samples_extracted_5UTR_coverage_dict):
    '''
    All_samples_extracted_5UTR_coverage_dict: sample id is the key.
    '''
    for i in assigned_indexes:
        ...
        for curr_5UTR_event_id in UTR_events_dict:
            curr_5UTR_structure = UTR_events_dict[curr_5UTR_event_id]
            curr_chr_local = curr_5UTR_structure[0]
            if curr_chr_local in curr_sample_All_chroms_coverage_dict:
                ...
                extracted_5UTR_region = curr_chr_coverage[0][left_region_index:right_region_index]
                extracted_5UTR_region.insert(0,region_start)
                extracted_5UTR_region.append(region_end)

                curr_event_info = [extracted_coverage,extracted_5UTR_region]
                All_samples_extracted_5UTR_coverage_dict[curr_5UTR_event_id,i] = curr_event_info
...
De_Novo_5UTR_Identification_Loading_Target_Wig_for_TCGA_Multiple_Samples_Multiple_threads_Main3_shared_list(sys.argv)
