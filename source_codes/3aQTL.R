...
cov = as.data.frame(fread(paste("/path/to/DICE/Imputed/QC_P3_R20.7_MAF0.05_HWE1E06_nodup.hg38_PC10.txt",sep=""),header = TRUE)) %>%
  ...
  .[,c("id",read.table("/path/to/DICE/StringTie_PB29/sample_list.txt", header = TRUE)[,1])]
...
IID_tr_g <- 
  ...
  .[read.table("/path/to/GEUV/StringTie_PB29/sample_list.txt", header = TRUE)[,1],] %>%
...
# normalized PDUI
utr="3UTR"
isopos_c = fread("/path/to/tools/DaPars2/data/geneInfo.txt") %>% 
  as.data.frame()
psi_c <- fread(paste0("/path/to/tools/DaPars2/normalized_PDUI/quantile-rank-normalized-PDUI_",utr,".txt")) %>%
  dplyr::rename(gid = query_name) %>%
  as.data.frame()
peer_factors <- fread(paste0("/path/to/tools/DaPars2/normalized_PDUI/PEERfactors_",utr,".txt")) %>%
  as.data.frame()
...
dir.create(paste0("/path/to/DICE/QTL/result/PDUI"), showWarnings = F, recursive = T)
...
for (stim in c("TFH","TH1","TH17","TH2","TH1-17","TREG_MEMORY","TREG_NAIVE","B_NAIVE","CD4_NAIVE","CD4_N_STIM","CD8_NAIVE","CD8_N_STIM","NONCLASSICAL_MONOCYTES","CLASSICAL_MONOCYTES","NK_CD16POS")){
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      write.table(snps, paste0("/path/to/snps_chr",chr,"_",utr,"-PDUI.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(expr2, paste0("/path/to/exp_chr",chr,"_",utr,"-PDUI.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(cov, paste0("/path/to/cov_chr",chr,"_",utr,"-PDUI.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      
      # Linear model to use, modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      useModel = modelLINEAR; # modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      
      # Genotype file name
      SNP_file_name = paste("/path/to/snps_chr",chr,"_",utr,"-PDUI.txt", sep="");
      
      # Gene expression file name
      expression_file_name = paste("/path/to/exp_chr",chr,"_",utr,"-PDUI.txt", sep="");
      
      # Covariates file name
      # Set to character() for no covariates
      covariates_file_name = paste("/path/to/cov_chr",chr,"_",utr,"-PDUI.txt", sep="");
      ...
      me$cis$eqtls %>%
        ...
        merge(.,
              isopos_c,
              by.x="gene",by.y="geneid") %>%
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/DICE/QTL/result/PDUI/chr",chr,"_in_",stim,"_",utr,"-PDUI.txt"),
...
dir.create(paste0("/path/to/EGA/QTL/result/PDUI"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/EGA/QTL/result/PDUI/chr",chr,"_in_",stimulus,"_",utr,"-PDUI.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
...
# ImmVar
dir.create(paste0("/path/to/ImmVar/QTL/result/PDUI"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/ImmVar/QTL/result/PDUI/chr",chr,"_in_",stim,"_",utr,"-PDUI.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
...
dir.create(paste0("/path/to/GEUV/QTL/result/PDUI"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/GEUV/QTL/result/PDUI/chr",chr,"_in_",stim,"_",utr,"-PDUI.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
