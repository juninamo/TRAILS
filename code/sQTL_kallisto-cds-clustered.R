...
isopos_c = fread("/path/to/eQTL/kallisto/geneInfo.txt") %>% 
  ...
  as.data.frame()
psi_c <- fread(paste0("/path/to/sQTL/kallisto_clustered-by-cds-vsearch/quantile-rank-normalized_transcript_ratio.txt")) %>%
  dplyr::rename(gid = V1) %>%
  as.data.frame()
peer_factors <- fread(paste0("/path/to/sQTL/kallisto_clustered-by-cds-vsearch/PEERfactors.txt")) %>%
  as.data.frame()
...
for (stim in c("TFH","TH1","TH17","TH2","TH1-17","TREG_MEMORY","TREG_NAIVE","B_NAIVE","CD4_NAIVE","CD4_N_STIM","CD8_NAIVE","CD8_N_STIM","NONCLASSICAL_MONOCYTES","CLASSICAL_MONOCYTES","NK_CD16POS")){
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      write.table(snps, paste0("/path/to/snps_chr",chr,"_kallisto-cds-clusterd-vsearch.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(expr2, paste0("/path/to/exp_chr",chr,"_kallisto-cds-clusterd-vsearch.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(cov, paste0("/path/to/cov_chr",chr,"_kallisto-cds-clusterd-vsearch.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      
      # Linear model to use, modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      useModel = modelLINEAR; # modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      
      # Genotype file name
      SNP_file_name = paste("/path/to/snps_chr",chr,"_kallisto-cds-clusterd-vsearch.txt", sep="");
      
      # Gene expression file name
      expression_file_name = paste("/path/to/exp_chr",chr,"_kallisto-cds-clusterd-vsearch.txt", sep="");
      
      # Covariates file name
      # Set to character() for no covariates
      covariates_file_name = paste("/path/to/cov_chr",chr,"_kallisto-cds-clusterd-vsearch.txt", sep="");
      ...
      me$cis$eqtls %>%
        ...
        write.table(., 
                    paste0("/path/to/DICE/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_kallisto-cds-clusterd-vsearch.txt"),
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/EGA/QTL/result/splicing_peer/chr",chr,"_in_",stimulus,"_kallisto-cds-clusterd-vsearch.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/ImmVar/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_kallisto-cds-clusterd-vsearch.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/GEUV/QTL/result/splicing_peer/chr",chr,"_in_",stim,"_kallisto-cds-clusterd-vsearch.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
