...
psi_c <- fread(paste0("/path/to/eQTL/kallisto/quantile-rank-normalized_gene_TPM_matrix.txt")) %>%
  dplyr::rename(gid = V1) %>%
  as.data.frame()
peer_factors <- fread(paste0("/path/to/eQTL/kallisto/PEERfactors.txt")) %>%
  as.data.frame()
...
dir.create(paste0("/path/to/DICE/QTL/result/cis_peer"), showWarnings = F, recursive = T)
...
for (stim in c("TFH","TH1","TH17","TH2","TH1-17","TREG_MEMORY","TREG_NAIVE","B_NAIVE","CD4_NAIVE","CD4_N_STIM","CD8_NAIVE","CD8_N_STIM","NONCLASSICAL_MONOCYTES","CLASSICAL_MONOCYTES","NK_CD16POS")){
  tryCatch({
    ...
    expr2 <- psi_c[, c("gid", shared$Run %>% as.character()) ] %>%
      .[ which ( .$gid %in% genepos[genepos$chr == paste0("chr",chr) & 
                                      genepos$s1 > min(snpspos$pos-(5e+05)) & 
                                      genepos$s2 < max(snpspos$pos+(5e+05)), "GENEID"] ), ]
    ...
    if ( nrow(expr2) > 0 &&
      ...
      write.table(snps, paste0("/path/to/snps_chr",chr,"_kallisto.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(expr2, paste0("/path/to/exp_chr",chr,"_kallisto.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      write.table(cov, paste0("/path/to/cov_chr",chr,"_kallisto.txt"), sep = "\t",quote=FALSE, row.names = FALSE)
      
      # Linear model to use, modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      useModel = modelLINEAR; # modelANOVA, modelLINEAR, or modelLINEAR_CROSS
      
      # Genotype file name
      SNP_file_name = paste("/path/to/snps_chr",chr,"_kallisto.txt", sep="");
      
      # Gene expression file name
      expression_file_name = paste("/path/to/exp_chr",chr,"_kallisto.txt", sep="");
      
      # Covariates file name
      # Set to character() for no covariates
      covariates_file_name = paste("/path/to/cov_chr",chr,"_kallisto.txt", sep="");
      ...
      me$cis$eqtls %>%
        ...
        write.table(., 
                    paste0("/path/to/DICE/QTL/result/cis_peer/chr",chr,"_in_",stim,"_kallisto.txt"),
...
dir.create(paste0("/path/to/EGA/QTL/result/cis_peer"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/EGA/QTL/result/cis_peer/chr",chr,"_in_",stimulus,"_kallisto.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
...
# ImmVar
dir.create(paste0("/path/to/ImmVar/QTL/result/cis_peer"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/ImmVar/QTL/result/cis_peer/chr",chr,"_in_",stim,"_kallisto.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
...
dir.create(paste0("/path/to/GEUV/QTL/result/cis_peer"), showWarnings = F, recursive = T)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/GEUV/QTL/result/cis_peer/chr",chr,"_in_",stim,"_kallisto.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
