...
plot_iso_structure <- function(gene = NULL, extra = 5e03, repeat_draw = TRUE, atac_draw = TRUE, rip_draw = TRUE, uorf_draw = FALSE){
  ...
  if (uorf_draw){
    ...
    uorf = rtracklayer::import.gff("../data/merged_uORF.gtf") %>%
      as.data.frame() %>%
      dplyr::filter(type == "exon") %>%
      dplyr::filter(transcript_id %in% isoform_Info[isoform_Info$associated_gene == gene, ]$isoform) %>%
      .[,c("seqnames","start","end","transcript_id")] %>%
      magrittr::set_colnames(c("chrom","start","end","isoform")) %>%
      dplyr::mutate(isoform = paste0("uORF [",isoform,"]"),
                    type = "uORF",
                    anno = "uORF",
                    domain = NA,
                    associated_gene = NA,
  ...
  if (uorf_draw){
    if (rip_draw){
      if (atac_draw){
        shared = intersect(colnames(coord_ia),colnames(coord_gencode_merged)) %>% intersect(.,colnames(domains)) %>% intersect(.,colnames(atac)) %>% intersect(.,colnames(rip)) %>% intersect(.,colnames(uorf))
      } else {
        shared = intersect(colnames(coord_ia),colnames(coord_gencode_merged)) %>% intersect(.,colnames(domains)) %>% intersect(.,colnames(rip)) %>% intersect(.,colnames(uorf))
      }
    } else {
      if (atac_draw){
        shared = intersect(colnames(coord_ia),colnames(coord_gencode_merged)) %>% intersect(.,colnames(domains)) %>% intersect(.,colnames(atac)) %>% intersect(.,colnames(uorf))
      } else {
        shared = intersect(colnames(coord_ia),colnames(coord_gencode_merged)) %>% intersect(.,colnames(domains)) %>% intersect(.,colnames(uorf))
      }  
    }
  } else {
    if (rip_draw){
          ...
          if (nrow(domains)>0 &
            ...
            g = rbind(coord_ia[,shared],coord_gencode_merged[,shared]) %>%
              ...
              geom_ribbon(data = . %>%
                          ...
                          alpha = 0.5) +
              geom_segment(data = . %>%
                             dplyr::filter(region == "uORF"),
                           aes(x = min(start), xend = max(end), y = 5, yend = 5), size = 0.5, linetype="dashed", color = "darkblue") +
              geom_rect(data = . %>%
                          dplyr::filter(region == "uORF"),
                        aes(xmin = start, xmax = end, ymin = 4, ymax = 6),
                        fill = "darkblue") +
              ggh4x::facet_nested(factor(anno,levels=c("RIP","uORF","Isoform Atlas","repetitive elements","GENCODE","ATAC")) + factor(isoform,levels = isoform_list) ~ ., scales = "free", nest_line = TRUE) +
          ...
          if (nrow(domains)>0 &
            ...
            g = rbind(coord_ia[,shared],coord_gencode_merged[,shared]) %>%
              ...
              geom_rect(data = . %>%
                        ...
                        aes(xmin = start, xmax = end, ymin = 4, ymax = 6),
                        fill = "darkblue") +
              ggh4x::facet_nested(factor(anno,levels=c("RIP","uORF","Isoform Atlas","repetitive elements","GENCODE")) + factor(isoform,levels = isoform_list) ~ ., scales = "free", nest_line = TRUE) +
              theme(strip.text.y = element_text(angle = 0)) +
              xlab(paste0("genomic position [", region_chrom, "]")) +
              ylab("") +
              labs(title = paste0("isoform structure [", gene, "]")) +
          ...
          if (nrow(domains)>0 &
            ...
            g = rbind(coord_ia[,shared],coord_gencode_merged[,shared]) %>%
              ...
              geom_rect(data = . %>%
                        ...
                        aes(xmin = start, xmax = end, ymin = 4, ymax = 6),
                        fill = "darkblue") +
              ggh4x::facet_nested(factor(anno,levels=c("uORF","Isoform Atlas","repetitive elements","GENCODE","ATAC")) + factor(isoform,levels = isoform_list) ~ ., scales = "free", nest_line = TRUE) +
              theme(strip.text.y = element_text(angle = 0)) +
              xlab(paste0("genomic position [", region_chrom, "]")) +
              ylab("") +
              labs(title = paste0("isoform structure [", gene, "]")) +
          ...
          if (nrow(domains)>0 &
            ...
            g = rbind(coord_ia[,shared],coord_gencode_merged[,shared]) %>%
              ...
              geom_rect(data = . %>%
                        ...
                        aes(xmin = start, xmax = end, ymin = 4, ymax = 6),
                        fill = "darkblue") +
              ggh4x::facet_nested(factor(anno,levels=c("uORF","Isoform Atlas","repetitive elements","GENCODE")) + factor(isoform,levels = isoform_list) ~ ., scales = "free", nest_line = TRUE) +
              theme(strip.text.y = element_text(angle = 0)) +
              xlab(paste0("genomic position [", region_chrom, "]")) +
              ylab("") +
              labs(title = paste0("isoform structure [", gene, "]")) +
