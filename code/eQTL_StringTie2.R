...
isopos_c = fread("/path/to/eQTL/StringTie_PB29/geneInfo.txt") %>% 
  merge(read2gene,.,by.x="query_name",by.y="transcript_id") %>%
  as.data.frame()
psi_c <- fread(paste0("/path/to/eQTL/StringTie_PB29/quantile-rank-normalized_gene_TPM_matrix.txt")) %>%
  dplyr::rename(gid = V1) %>%
  as.data.frame()
peer_factors <- fread(paste0("/path/to/eQTL/StringTie_PB29/PEERfactors.txt")) %>%
  as.data.frame()
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        write.table(., 
                    paste0("/path/to/DICE/QTL/result/cis_peer/chr",chr,"_in_",stim,"_StringTie.txt"),
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/EGA/QTL/result/cis_peer/chr",chr,"_in_",stimulus,"_StringTie.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/ImmVar/QTL/result/cis_peer/chr",chr,"_in_",stim,"_StringTie.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
  ...
  tryCatch({
    ...
    if ( nrow(expr2) > 0 &&
      ...
      me$cis$eqtls %>%
        ...
        dplyr::arrange(pvalue) %>%
        write.table(., 
                    paste0("/path/to/GEUV/QTL/result/cis_peer/chr",chr,"_in_",stim,"_StringTie.txt"),
                    sep = "\t",quote=FALSE, row.names = FALSE)
